...
void FIR_CreateImpulse(double *freqs, double *impulse, int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        double window = 0.54 - 0.46 * cos(2.0 * M_PI * i / (count - 1));
        impulse[i] *= window;
    }
}
...
bool FIR_CreateFilterByFreqs(FIR_Filter *filter, double *freqs, int count)
{
    ...
    int filterSize = count * sizeof(double);
    ...
    filter->originalSignal = calloc(signalLength, sizeof(double));
    filter->outputSignal = calloc(signalLength, sizeof(double));
...
double FIR_Next(FIR_Filter *filter, double input)
{
    ...
    double toReturn = filter->outputSignal[offsetMiddle];

    // Pointers
    double *impulse = filter->impulse;
    double *originalSignal = filter->originalSignal;
    double *outputSignal = filter->outputSignal;
    ...
    double last = originalSignal[offsetLast];
